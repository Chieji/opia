...
class TogetherProvider(BaseProvider):
    ...
    def __init__(self, name: str = "together", base_url: str = "https://api.together.xyz/v1"):
        self.name = name
        self.base_url = base_url
        self._default_model = "togethercomputer/llama-3-70b-instruct"
    ...
    async def chat(self, messages: list[Message], model: str, **kwargs) -> ChatResponse:
        ...
        async with httpx.AsyncClient(timeout=60) as client:
            ...
            if r.status_code == 401:
                raise RuntimeError("Invalid Together AI API key")
            r.raise_for_status()
            data = r.json()
            return ChatResponse(
    ...
    async def embeddings(self, input, model: str, **kwargs) -> EmbeddingsResponse:
        raise NotImplementedError("Together AI embeddings require a specific model")
